...
escribe_expresionBin(char *elem1, char*elem2, char*op, char*tip){
	...
	genera_temporal();
		if(en_fun==0){
			fprintf(fichero1,"%s %s;\n",tip,temp);
			if((tip=="pila_entero")||(tip=="pila_real")||(tip=="pila_booleano")||(tip=="pila_caracter")){
				if(tip=="pila_entero"){
					fprintf(fichero1,"inicializa_entero_sin_parametros(&%s);\n",temp);
				}else 
					if(tip=="pila_real"){
						fprintf(fichero1,"inicializa_real_sin_parametros(&%s);\n",temp);
				}else 
					if(tip=="pila_booleano"){
						fprintf(fichero1,"inicializa_booleano_sin_parametros(&%s);\n",temp);
				}else 
					if(tip=="pila_caracter"){
						fprintf(fichero1,"inicializa_caracter_sin_parametros(&%s);\n",temp);
					}
				if(op=="ins") escribe_insertar(tip,temp,elem2);
			}else{
	...
	if(en_fun==0){
		...
		if((tip=="pila_entero")||(tip=="pila_real")||(tip=="pila_booleano")||(tip=="pila_caracter")){
				if(tip=="pila_entero"){
					...
					if(op=="sac") escribe_sacar_pila(tip,temp);
					if(op=="top")	escribe_tope_pila(tip,temp);
					//if(op=="!")	escribe_pila_vacia(tip,temp);
			}else{
				fprintf(fichero1,"%s %s;\n",tip,temp);
...
void escribe_argumentos(){
	...
	for(i=indiceProc()+1;i<=tope;i++){
		if(primero==1){
			fprintf(fichero2,"%s",TipoEscrito(TS[i].tipoDato)); 
			primero =0;
		}
		else{
			fprintf(fichero2,", %s",TipoEscrito(TS[i].tipoDato)); 	
		}
		fprintf(fichero2," %s",TS[i].lexema); 
	}
	fprintf(fichero2,")"); 
}
...
void add_const(char *le){
	pila_ind++;
	cons_pila[pila_ind].elem_pila=le;
}
...
*/
void dec_cons_pila(char* tipo, char* id){
	if(en_fun==0){
		if(tipo=="pila_entero"){
			fprintf(fichero1,"inicializa_entero_sin_parametros(&%s)",id);
		}else 
			if(tipo=="pila_real"){
				fprintf(fichero1,"inicializa_real_sin_parametros(&%s)",id);
			}else
				if(tipo=="pila_booleano"){
					fprintf(fichero1,"inicializa_booleano_sin_parametros(&%s)",id);	
				}else
					if(tipo=="pila_caracter"){
						fprintf(fichero1,"inicializa_caracter_sin_parametros(&%s)",id);	
					}
		fprintf(fichero1,";\n");
	}else{
		if(tipo=="pila_entero"){
...
void escribe_insertar(char* tipo,char* p1,char* e){
	if(en_fun==0){
		if(tipo=="pila_entero"){
			fprintf(fichero1,"%s=insertar_entero(&%s,%s);\n",p1,p1,e);
		}else 
			if(tipo=="pila_real"){
				fprintf(fichero1,"%s=insertar_real(&%s,%s);\n",p1,p1,e);
			}else
				if(tipo=="pila_booleano"){
					fprintf(fichero1,"%s=insertar_booleano(&%s,%s);\n",p1,p1,e);
				}else
					if(tipo=="pila_caracter"){
						fprintf(fichero1,"%s=insertar_caracter(&%s,%s);\n",p1,p1,e);	
					}
	}else{
		if(tipo=="pila_entero"){
			fprintf(fichero2,"%s=insertar_entero(&%s,%s);\n",p1,p1,e);
		}else 
			if(tipo=="pila_real"){
				fprintf(fichero2,"%s=insertar_real(&%s,%s);\n",p1,p1,e);
			}else
				if(tipo=="pila_booleano"){
					fprintf(fichero2,"%s=insertar_booleano(&%s,%s);\n",p1,p1,e);
				}else
					if(tipo=="pila_caracter"){
						fprintf(fichero2,"%s=insertar_caracter(&%s,%s);\n",p1,p1,e);	
					}
	}
}

void escribe_sacar_pila(char* tipo,char* p1){
	if(en_fun==0){
		if(tipo=="pila_entero"){
			fprintf(fichero1,"%s=sacar_entero(&%s);\n",p1,p1);
		}else 
			if(tipo=="pila_real"){
				fprintf(fichero1,"%s=sacar_real(&%s);\n",p1,p1);
			}else
				if(tipo=="pila_booleano"){
					fprintf(fichero1,"%s=sacar_booleano(&%s);\n",p1,p1);
				}else
					if(tipo=="pila_caracter"){
						fprintf(fichero1,"%s=sacar_caracter(&%s);\n",p1,p1);	
					}
	}else{
		if(tipo=="pila_entero"){
			fprintf(fichero2,"%s=sacar_entero(&%s);\n",p1,p1);	
		}else 
			if(tipo=="pila_real"){
				fprintf(fichero2,"%s=sacar_real(&%s);\n",p1,p1);	
			}else
				if(tipo=="pila_booleano"){
					fprintf(fichero2,"%s=sacar_booleano(&%s);\n",p1,p1);	
				}else
					if(tipo=="pila_caracter"){
						fprintf(fichero2,"%s=sacar_caracter(&%s);\n",p1,p1);		
					}
	}
}

void escribe_tope_pila(char* tipo,char* p1){
	if(en_fun==0){
		if(tipo=="pila_entero"){
			fprintf(fichero1,"%s=tope_entero(%s);\n",p1,p1);
		}else 
			if(tipo=="pila_real"){
				fprintf(fichero1,"%s=tope_real(%s);\n",p1,p1);
			}else
				if(tipo=="pila_booleano"){
					fprintf(fichero1,"%s=tope_booleano(%s);\n",p1,p1);
				}else
					if(tipo=="pila_caracter"){
						fprintf(fichero1,"%s=tope_caracter(%s);\n",p1,p1);	
					}
	}else{
		if(tipo=="pila_entero"){
			fprintf(fichero2,"%s=sacar_entero(&%s);\n",p1,p1);	
...
void escribe_sumres_pp(char* p1, char* p2, dTipo tipo){
	copiaToTemp1(p1);
	copiaToTemp2(p2);
	genera_temporal();
	...
	if(tipo==0){
		fprintf(fichero1,"%s %s;\n",tipo,temp);
		fprintf(fichero1,"%s=suma_pp_entero(%s,%s);\n",temp,p1,p2);
	}
}
